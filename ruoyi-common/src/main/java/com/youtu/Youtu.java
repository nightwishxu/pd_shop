...
public class Youtu {
	...
	public JSONObject BankCardOcr(String image_path) throws IOException,
            JSONException, KeyManagementException, NoSuchAlgorithmException {
		StringBuffer image_data = new StringBuffer("");
		JSONObject data = new JSONObject();
		...
		GetBase64FromFile(image_path, image_data);
		data.put("image", image_data.toString());
		JSONObject respose =m_not_use_https?SendHttpRequest(data, "ocrapi/creditcardocr"):SendHttpsRequest(data, "ocrapi/creditcardocr");
		return respose;
	}

	public JSONObject textToAudio(String text, int model_type, int speed) throws IOException,
            JSONException, KeyManagementException, NoSuchAlgorithmException {
		JSONObject data = new JSONObject();

		data.put("text", text);
		data.put("model_type", model_type);
		data.put("speed",speed);
		JSONObject respose =m_not_use_https?SendHttpRequest(data, "ttsapi/text_to_audio"):SendHttpsRequest(data, "ttsapi/text_to_audio");
		return respose;
	}
